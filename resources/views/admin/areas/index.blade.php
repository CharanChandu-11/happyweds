...
<div class="container-fluid py-5 page-spacing font-sans" style="max-width: 1100px;">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3 animate-card">
        <div>
            ...
            <p class="text-muted mt-2 mb-0" style="font-size: 1rem; font-weight: 500;">Manage geographic locations, cities, and states for profiles.</p>
        </div>
    </div>
    ...
    <div class="premium-card p-4 mb-5 animate-card delay-1">
        <form method="POST" action="{{ route('admin.areas.store') }}">
            ...
            <div class="row g-3 align-items-end">
                <div class="col-md-5">
                    <label class="premium-label">Area / City Name</label>
                    <div class="position-relative">
                        <i class="bi bi-geo-alt position-absolute text-muted fs-5" style="left: 15px; top: 12px;"></i>
                        <input type="text" name="area" class="form-control premium-input w-100" style="padding-left: 45px;" placeholder="e.g. Mumbai, Hyderabad" required>
                    </div>
                </div>
                <div class="col-md-4">
                    <label class="premium-label">State Name</label>
                    <div class="position-relative">
                        <i class="bi bi-map position-absolute text-muted fs-5" style="left: 15px; top: 12px;"></i>
                        <input type="text" name="state" class="form-control premium-input w-100" style="padding-left: 45px;" placeholder="e.g. Maharashtra, Telangana" required>
                    </div>
                </div>
                <div class="col-md-3">
                ...
                <tbody>
                    @forelse($data as $row)
                        <tr>
                            ...
                            <td>
                                <span class="fw-bold text-secondary fs-6 text-capitalize">
                                    {{ $row->state }}
                                </span>
                            </td>
                            ...
                            <td class="text-end pe-4">
                                <form action="{{ route('admin.areas.destroy', $row->id) }}" method="POST" class="m-0 p-0" onsubmit="return confirm('Are you sure you want to delete this location record?')">
                                    @csrf 
                                    @method('DELETE')
                                    <button type="submit" class="action-icon-btn delete" title="Delete Location">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-5 bg-transparent shadow-none border-0">
                                <div class="d-flex flex-column align-items-center justify-content-center py-4">
                                    <div class="rounded-circle bg-white shadow-sm d-flex align-items-center justify-content-center mb-4" style="width: 80px; height: 80px;">
                                        <i class="bi bi-pin-map-fill text-muted" style="font-size: 2.5rem;"></i>
                                    </div>
                                    <h4 class="font-serif fw-bold text-dark">No Locations Found</h4>
                                    <p class="text-muted fw-medium fs-6">Use the form above to add your first city and state.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
